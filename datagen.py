...
def gsm8k_test(model, data_path, start=0, end=MAX_INT, batch_size=1, tensor_parallel_size=1):
    ...
    problem_prompt = (
        "### Instruction:\n{instruction}. Give your answer within {i} tokens \n\n### Response: Let's think step by step."
    )
    # print('promt =====', problem_prompt)
    with open(data_path,"r+", encoding="utf8") as f:
        for idx, item in enumerate(jsonlines.Reader(f)):
            temp_instr = problem_prompt.format(instruction=item["query"], i = random.choice(numbers))
    ...
    dataset_path = "/data/siqizhu/llama_outputs.json"
