...
'''
/data/siqizhu/MetaMath$ python merge_lora_with_llama.py \
    --base_model /data/dataset/llama/llama-2-7b-chat-hf/ \
    --lora_model /data/siqizhu/merged_model_mar04/ \
    --output_type huggingface \
    --output_dir /data/siqizhu/llama-2-7b-chat-lora-math-mar04/

/data/siqizhu/MetaMath$ python merge_lora_with_llama.py \
    --base_model /mnt/data/zhongrx/llama-2-7b-hf/ \
    --lora_model /data/siqizhu/merged_model_llama2/ \
    --output_type huggingface \
    --output_dir /data/siqizhu/llama-2-7b-lora-math/
'''
import argparse
import json
import os
...
if __name__=='__main__':
    ...
    lora_model_paths = [
        os.path.join(args.lora_model, f)
        for f in os.listdir(args.lora_model)
    ]
    ...
    for lora_index, lora_model_path in enumerate(lora_model_paths):
        ...
        if hasattr(peft.LoraModel,'merge_and_unload'):
            ...
            try:
                lora_model_sd = torch.load(os.path.join(lora_model_path,'adapter_model.safetensors'),map_location='cpu')
            except FileNotFoundError:
                print("Cannot find lora model on the disk. Downloading lora model from hub...")
                filename = hf_hub_download(repo_id=lora_model_path,filename='adapter_model.safetensors')
        ...
        output_dir1 = os.path.join(output_dir, f"lora_{lora_index}")
        tokenizer.save_pretrained(output_dir1)

        if output_type=='huggingface':
            print("Saving to Hugging Face format...")
            LlamaForCausalLM.save_pretrained(
                base_model, output_dir1,
                max_shard_size="2GB"
